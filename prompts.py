...
SYSTEM_PROMPT = "Вы - опытный преподаватель математики, специализирующийся на подготовке к ЕГЭ. Давайте точные и краткие ответы без лишних пояснений. Даже, если что-то не понятно в условии, то всё равно надо дать полный ответ - дополнительных пояснений не будет"
...
2. Параметры в зависимости от типа:
   ...
   Для типа "четырехугольник":
   - Тип: [прямоугольник/квадрат/ромб/произвольный]
   - Размеры: [ширина и высота для прямоугольника, длина стороны для квадрата и т.д.]
   - Координаты: [координаты левого нижнего угла, например "(2,3)"]
   - Подписи вершин: [укажите имена вершин через запятую, например "A,B,C,D". Подписывайте ТОЛЬКО те вершины, которые упомянуты в условии задачи]
   ...
   - Точки: [перечислите точки с их названиями, например "A(1,2),B(3,4),C(5,6)". Указывайте метки ТОЛЬКО для тех точек, которые упомянуты в условии задачи. Для остальных точек используйте только координаты: (1,2), (3,4)]
   - Векторы: [перечислите векторы, например "AB, BC" если они названы в условии, или используйте координаты "(1,2,3,4)" для безымянных векторов]
   - Функции: [перечислите функции для отображения, например "x**2, 2*x+1"]

3. Дополнительные указания:
   ВАЖНО: Подписывайте ТОЛЬКО те элементы, которые непосредственно упомянуты в условии задачи. Например, если в задаче говорится о точке A(1,2), то подпишите её. Если говорится о треугольнике ABC, подпишите все его вершины. Если условие содержит "точка O - центр окружности", то подпишите центр как O. 
