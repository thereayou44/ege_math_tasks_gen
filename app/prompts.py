...
- КЛЮЧЕВОЕ ТРЕБОВАНИЕ: ТИП ЗАДАЧИ ДОЛЖЕН СТРОГО СООТВЕТСТВОВАТЬ КАТЕГОРИИ И ПОДКАТЕГОРИИ! Не меняйте тип задачи - меняйте только числовые параметры, коэффициенты и множители. Тип задания (тригонометрия, логарифмы, и т.д.) должен оставаться неизменным.
- КЛЮЧЕВОЕ ТРЕБОВАНИЕ: МАКСИМАЛЬНОЕ РАЗНООБРАЗИЕ! Каждая новая задача ДОЛЖНА существенно отличаться от предыдущих версий в плане математических функций, структуры и параметров. Одна и та же функция или структура НЕ ДОЛЖНА повторяться при повторной генерации.
- Вы можете делать математические изменения в задачах (например, заменять косинус на синус, логарифм с основанием 2 на логарифм с основанием 3, менять знаки неравенств и т.п.), главное - сохранять корректность задачи и примерно тот же уровень сложности - при каждой новой генерации задачи она должна отличаться от предыдущей.
- ВАЖНО: Генерируйте РАЗНООБРАЗНЫЕ задачи. Для каждой новой генерации меняйте не только числа, но и структуру/форму задачи, но так, чтобы сложность и корректность задачи осталось примерно такой же. Например:
  * Меняйте тригонометрические функции (sin на cos, tg на ctg)
  * Меняйте коэффициенты перед аргументами (sin(2x) на sin(3x))
  * Меняйте множители (2sin(x) на 3sin(x))
  * Используйте разные комбинации функций (sin(x) на sin(x)+cos(x))
  * Меняйте аргументы (sin(x) на sin(x+π/4))
  * Меняйте композиции функций (sin²(x) на sin(x²))
  * Меняйте другие параметры задачи (интервалы, ограничения, условия)
- Используйте только целые числа, простые дроби и корни из целых чисел.
- Форматируй задачу красиво и максимально читаемо. Всё на русском языке
...
- При создании параметров визуализации всегда указывайте ТОЛЬКО те параметры, которые ЯВНО УПОМИНАЮТСЯ в условии задачи. НЕ ДОБАВЛЯЙТЕ ТОЧКИ на графики функций, если эти точки не указаны явно в условии задачи. Например, если в задаче сказано "На рисунке изображен график функции f(x) = x^2 - 4x + 3", не нужно добавлять никаких особых точек - только саму функцию.
- При указании функций для визуализации используйте выражения, совместимые с matplotlib/numpy. Например, вместо \pi используйте pi (система автоматически преобразует это в np.pi). Для тригонометрических функций используйте sin(x), cos(x) вместо \sin(x), \cos(x).
...
def check_visualization_requirement(category, subcategory, task_text):
    """
    Определяет, нужно ли для этой категории/подкатегории задачи запрашивать визуализацию.
    
    Args:
        category (str): Категория задачи
        subcategory (str): Подкатегория задачи
        task_text (str): Текст задачи
        
    Returns:
        bool: True если нужно запрашивать визуализацию, False в противном случае
    """
    # Геометрические задачи часто требуют визуализации
    geometric_categories = [
        "геометрия", "планиметрия", "треугольники", "окружности", 
        "многоугольники", "стереометрия", "координатная плоскость"
    ]
    
    if any(gc.lower() in category.lower() for gc in geometric_categories):
        return True
    
    # Категории с графиками функций
    graph_categories = [
        "функции и графики", "график", "чтение графиков", "исследование функций", 
        "производная", "интеграл", "диаграмма"
    ]
    
    if any(gc.lower() in category.lower() for gc in graph_categories):
        return True
    
    # Проверка по подкатегории
    if subcategory:
        if any(gc.lower() in subcategory.lower() for gc in geometric_categories + graph_categories):
            return True
    
    # Проверка текста задачи на ключевые слова
    visual_keywords = [
        "график", "диаграмма", "рисунок", "точка", "на рисунке", "изображен", 
        "показан", "окружность", "треугольник", "прямоугольник", "параллелограмм",
        "трапеция", "фигура", "система координат", "угол", "проекция", "сечение"
    ]
    
    if any(kw.lower() in task_text.lower() for kw in visual_keywords):
        return True
    
    # ВАЖНО: Возвращаем True, чтобы всегда запрашивать визуализацию и позволить модели решить
    # нужна ли визуализация для конкретной задачи, независимо от уровня (базовый или профильный)
    return True
