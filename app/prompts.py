...
При изменении регулярных выражений в REGEX_PATTERNS следите за тем, чтобы скобки, 
которые не являются группами захвата, были экранированы с помощью \( и \).
Регулярные выражения должны содержать только одну группу захвата для извлечения значения.
"""
# Системный промпт для Yandex API
SYSTEM_PROMPT = """Вы - опытный преподаватель математики с многолетним стажем подготовки к ЕГЭ. Ваша задача - создавать уникальные и математически корректные задачи для подготовки учащихся.

КЛЮЧЕВЫЕ ТРЕБОВАНИЯ К ГЕНЕРИРУЕМЫМ ЗАДАЧАМ:

1. МАТЕМАТИЧЕСКАЯ КОРРЕКТНОСТЬ:
   - Каждая задача ДОЛЖНА иметь однозначное решение и конкретный числовой ответ
   - Исключены задачи с ответом "решений нет" или "любое число"
   - Все формулы должны быть математически верны, все вычисления проверены
   - Используйте строгую математическую терминологию, принятую в российском образовании

2. РАЗНООБРАЗИЕ И УНИКАЛЬНОСТЬ:
   - КАЖДАЯ новая генерация даже по одной теме должна СУЩЕСТВЕННО отличаться от предыдущих
   - Меняйте контексты, числовые данные, типы функций, формы представления
   - Для уравнений: меняйте коэффициенты, функции (sin → cos, log₃ → log₅), знаки
   - Для геометрических задач: меняйте размеры, конфигурации, искомые элементы
   - Избегайте повторения одних и тех же числовых значений в разных задачах

3. СТРУКТУРА И ПОЛНОТА:
   - ОБЯЗАТЕЛЬНО включайте: условие задачи, полное решение, подсказки, числовой ответ
   - Решение должно быть максимально подробным, с обоснованием каждого шага
   - Финальный ответ должен быть четко обозначен в конце решения
   - Ответ должен быть представлен в наиболее упрощенной форме (сокращенная дробь, упрощенный корень)

4. УРОВЕНЬ СЛОЖНОСТИ:
   - Задачи должны соответствовать уровню ЕГЭ по математике - не слишком простые, но и не выходящие за рамки школьной программы
   - Используйте преимущественно "красивые" числа в ответах: целые числа, простые дроби (1/2, 3/4), корни из небольших чисел (√2, √3)
   - Избегайте чрезмерно громоздких вычислений и неочевидных трюков при решении

5. ТИПЫ ЗАДАЧ И ИХ ОСОБЕННОСТИ:
   - Алгебра: уравнения и неравенства должны иметь решение без перебора огромного количества случаев
   - Геометрия: задачи должны иметь корректный чертеж и однозначные данные
   - Текстовые задачи: сюжет должен быть реалистичным и понятным
   - Задачи с параметром: исследуйте все случаи, но не усложняйте анализ излишне

Даже при отсутствии примера "как должна выглядеть задача", создавайте задачу полностью соответствующую указанной теме, с интересным содержанием и уникальными числовыми данными.

ОЧЕНЬ ВАЖНО: каждая задача ВСЕГДА должна иметь решение и числовой ответ - без исключений."""
...
# Список категорий, для которых может потребоваться визуализация
VISUALIZATION_CATEGORIES = [
    # Графики и функции
    "график", "функц", "диаграмм", "чтение графиков", "анализ график", 
    "график функц", "исследование функций", "производная", "первообразная",
    
    # Планиметрия
    "планиметр", "геометр", "треугольник", "окружност", "многоугольник", 
    "четырехугольник", "четырёхугольник", "прямоугольник", "квадрат", 
    "трапец", "параллелограмм", "ромб", "плоскост", "координат",
    
    # Стереометрия
    "стереометр", "объем", "площад", "куб", "призм", "пирамид", "конус",
    "цилиндр", "шар", "параллелепипед", "многогранник", "круглые тела", "сечен",
    
    # Разное
    "вектор", "координат", "квадратная решетк", "квадратная решётк"
]
...
def create_complete_task_prompt(category, subcategory, original_task, difficulty_level=3, add_visualization=None):
    ...
    Args:
        ...
        add_visualization: Флаг для добавления инструкций для визуализации
                          None - определять автоматически по категории
                          True - всегда добавлять
                          False - никогда не добавлять
    ...
    # Генерируем инструкции для разнообразия задач в зависимости от категории
    diversity_instructions = """
---ТРЕБОВАНИЯ К РАЗНООБРАЗИЮ ЗАДАЧИ---
ВАЖНО: Генерируйте ПРИНЦИПИАЛЬНО НОВУЮ задачу, отличную от исходной и от предыдущих вариантов, НО СТРОГО ТОГО ЖЕ ТИПА в соответствии с категорией/подкатегорией. Это КРИТИЧНО для эффективного обучения. Сохраняйте тот же тип задачи и уровень сложности, но ОБЯЗАТЕЛЬНО меняйте:

1. КОЭФФИЦИЕНТЫ И ПАРАМЕТРЫ - меняйте числовые коэффициенты, множители и свободные члены 
   (например, 2cos(3x) на 3cos(2x), 2x²+3x-5 на -x²+4x-1)
2. Числовые данные и диапазоны в условии задачи
3. Формулировку условия и контекст, НО не меняя тип задачи
4. Вид и представление математических объектов ПРИ СОХРАНЕНИИ их типа

Помните: простая замена чисел НЕ ЯВЛЯЕТСЯ достаточным изменением. Необходимо менять структуру выражений,
но сохраняя математическую корректность, общий тип задачи и примерно тот же уровень сложности.

"""

    # Добавляем специфические инструкции для разных категорий задач
    if "триго" in category.lower() or "триго" in subcategory.lower():
        diversity_instructions += """
Для тригонометрических задач:
- В РАМКАХ ОДНОГО ТИПА ТРИГОНОМЕТРИЧЕСКИХ ФУНКЦИЙ: можно заменять cos на sin, tg на ctg и наоборот, если это не меняет ПРИНЦИПИАЛЬНО тип задачи
- ОБЯЗАТЕЛЬНО: меняйте коэффициенты перед x в тригонометрических функциях: 
  * Например, если есть y = 2*cos(3x) + 1, сделайте y = 3*cos(2x) - 2 или y = 4*cos(πx/2) + 3
  * Если есть sin(x), попробуйте sin(2x), sin(3x), sin(x/2) или sin(πx)
  * Если есть формула вида a*sin(bx+c), меняйте ВСЕ коэффициенты a, b и c на другие значения
- ОБЯЗАТЕЛЬНО: меняйте коэффициенты перед тригонометрическими функциями:
  * Если задача содержит 2*sin(x), замените на 3*sin(x) или -sin(x) или 1/2*sin(x)
- Если задача на тригонометрические уравнения, сохраняйте тип уравнения, но меняйте коэффициенты и свободные члены
- Меняйте аргументы внутри функций: sin(x) → sin(x+π/4) → sin(x-π/3)
- Меняйте вид тригонометрических формул: произведение на сумму, двойной угол на основную форму и т.д.
"""

    if "функц" in category.lower() or "график" in category.lower() or "функц" in subcategory.lower() or "график" in subcategory.lower():
        diversity_instructions += """
Для задач с функциями:
- СТРОГО СОХРАНЯЙТЕ ТИП ФУНКЦИИ из категории/подкатегории:
  * Если задача в категории "показательные функции", используйте ТОЛЬКО показательные функции
  * Если задача в категории "логарифмические функции", используйте ТОЛЬКО логарифмические функции
  * Если задача в категории "Параболы", используйте ТОЛЬКО квадратичные функции
- ОБЯЗАТЕЛЬНО: меняйте коэффициенты в функциях:
  * При каждой генерации используйте ДРУГИЕ числовые значения для всех параметров
  * Например, для полинома y = 2x² - 3x + 1 используйте y = -x² + 4x - 2 или y = 3x² + x - 5
  * Для показательной y = 2ˣ + 1 используйте y = 3ˣ - 2 или y = -2ˣ + 3
- Меняйте знаки коэффициентов: f(x) = 2x² - 4x + 1 на f(x) = -3x² + 2x - 5
- Меняйте аргументы: log(x+1) → log(2x-3), eˣ → e^(x-1)
- При необходимости, добавляйте или убирайте смещения: f(x) = 2ˣ на f(x) = 2ˣ + 3
- Меняйте интервалы, на которых рассматривается функция
"""

    if "урав" in category.lower() or "нерав" in category.lower() or "урав" in subcategory.lower() or "нерав" in subcategory.lower():
        diversity_instructions += """
Для уравнений и неравенств:
- Меняйте вид уравнения, сохраняя тип: линейное, квадратное, иррациональное, показательное
- Меняйте условие на противоположное: уравнение на неравенство, ">" на "<"
- Меняйте коэффициенты и свободные члены
- Добавляйте или убирайте модули, параметры, степени
- Варьируйте типы систем уравнений: линейные, нелинейные, смешанные
- Меняйте методы решения: замена переменной, разложение на множители, метод интервалов
"""

    if "геометр" in category.lower() or "планиметр" in category.lower() or "стереометр" in category.lower() or "геометр" in subcategory.lower():
        diversity_instructions += """
Для геометрических задач:
- Меняйте тип фигуры: треугольник на четырехугольник, круг на эллипс
- Меняйте свойства фигур: прямоугольный треугольник на равнобедренный
- Меняйте размеры и соотношения: длины сторон, радиусы, углы
- Меняйте положение фигур: координаты на плоскости, взаимное расположение
- Меняйте искомые величины: площадь, периметр, объем, угол
- Меняйте дополнительные элементы: высоты, медианы, диагонали, сечения
"""

    if "вероятн" in category.lower() or "комбин" in category.lower() or "статист" in category.lower() or "вероятн" in subcategory.lower():
        diversity_instructions += """
Для задач по теории вероятностей и комбинаторике:
- Меняйте типы комбинаторных объектов: перестановки, размещения, сочетания
- Меняйте условия отбора: с повторениями/без повторений, с учетом порядка/без учета
- Меняйте вероятностные эксперименты: монеты на кубики, карты на шары
- Меняйте количество элементов и исходов
- Меняйте типы событий: независимые/зависимые, совместные/несовместные
- Варьируйте виды вероятностных распределений
"""

    if "экономич" in category.lower() or "финансов" in category.lower() or "экономич" in subcategory.lower():
        diversity_instructions += """
Для экономических задач:
- Меняйте типы экономических показателей: прибыль, кредит, вклад, налог
- Меняйте процентные ставки, начальные суммы, сроки
- Меняйте условия: простые/сложные проценты, аннуитетные/дифференцированные платежи
- Меняйте экономические сценарии: покупка, продажа, инвестиции, кредиты
- Варьируйте периоды начисления: ежемесячно, ежеквартально, ежегодно
"""

    # Определяем, нужно ли добавлять запрос о генерации параметров для изображения
    image_prompt = ""
    
    # Определяем, требуется ли визуализация для данной категории
    if add_visualization is None:
        # Автоматически определяем по категории и подкатегории
        category_lower = category.lower()
        subcategory_lower = subcategory.lower() if subcategory else ""
        
        add_visualization = False
        # Проверяем, содержит ли категория или подкатегория ключевые слова из списка
        for keyword in VISUALIZATION_CATEGORIES:
            if keyword in category_lower or keyword in subcategory_lower:
                add_visualization = True
                break
        
        # Проверим также текст задачи на наличие упоминаний о графиках, фигурах и т.д.
        if not add_visualization:
            visualization_keywords = ["график", "рисунок", "изображен", "показан", "треугольник", 
                                     "окружность", "прямоугольник", "трапеция", "система координат", 
                                     "плоскость", "кривая", "диаграмма", "фигура"]
            for keyword in visualization_keywords:
                if keyword in original_task.lower():
                    add_visualization = True
                    break
...
ВАЖНО: Используйте ТОЛЬКО те параметры для визуализации, которые ЯВНО УПОМИНАЮТСЯ в условии задачи. НЕ ДОБАВЛЯЙТЕ никаких дополнительных элементов, которых нет в задаче.

ОСОБЕННО ВАЖНО ДЛЯ ГРАФИКОВ ФУНКЦИЙ: НЕ ДОБАВЛЯЙТЕ никаких особых точек на график, если они НЕ УКАЗАНЫ ЯВНО в условии задачи прямым текстом. Если в задаче просто сказано "На рисунке изображен график функции f(x)", то НЕ НУЖНО добавлять никаких особых точек - оставьте параметр "Особые точки" ПУСТЫМ или вообще не указывайте его.
...
1. Задача с анализом графика функции без особых точек:
Тип: график
...
2. Задача с анализом графика функции с особыми точками, ЯВНО указанными в условии задачи:
Тип: график
Количество функций: 1
...
Диапазон X: [-2, 8]
Диапазон Y: [-5, 5]
Особые точки: (1,0,A), (3,0,B), (4,0,C)
...
---ИНСТРУКЦИИ---
На основе приведенной задачи создайте новую задачу с решением. 
Задача должна иметь тот же характер и уровень сложности, но быть существенно отличной от исходной. 
Результат представьте в следующем формате:

---ЗАДАЧА---
[Текст новой задачи]

---РЕШЕНИЕ---
[Полное пошаговое решение новой задачи]

{hint_instructions}
{image_prompt if add_visualization else ""}
"""
...
def check_visualization_requirement(category, subcategory, task_text):
    """
    Определяет, необходима ли визуализация для задачи.
    ...
    Returns:
        bool: True, если требуется визуализация
    """
    # Проверяем категорию и подкатегорию на ключевые слова из списка
    category_lower = category.lower()
    subcategory_lower = subcategory.lower() if subcategory else ""
...
def create_new_task_prompt(original_task, category, subcategory=""):
    """
...
Используй только $ или $$ для формул
"""
...
REGEX_PATTERNS = {
    ...
    "graph": {
        ...
        "special_points": r'Особые точки\s*\(?только те, что прямым текстом указаны в условии\)?[^:]*:\s*["\']?([^"\'\n]+)["\']?',
        "special_points_coords": r'Координаты особых точек\s*\(?только те, что прямым текстом указаны в условии\)?[^:]*:\s*["\']?([^"\'\n]+)["\']?'
    },
    "coordinate": {
        "points": r'Точки[^:]*:\s*["\']?([^"\'\n]+)["\']?',
        "vectors": r'Векторы[^:]*:\s*["\']?([^"\'\n]+)["\']?',
