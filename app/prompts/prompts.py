...
- Для каждого шага, где используется формула или утверждение, обязательно указывайте её название (например: "По теореме Пифагора...", "Используя формулу площади треугольника...", "По свойству логарифма произведения...")
- Если в задаче есть ограничения на переменную (ОДЗ), обязательно выписывайте и учитывайте их в решении
...
- ОБЯЗАТЕЛЬНО укажите о необходимости выписать ОДЗ, если в задаче есть ограничения на переменную (например, логарифмы, дроби, корни чётной степени)
...
# Требования ЕГЭ для подсказок
HINT_EGE_REQUIREMENTS = """
ТРЕБОВАНИЯ ЕГЭ ДЛЯ ПЕРВОЙ ПОДСКАЗКИ:
Первая подсказка ОБЯЗАТЕЛЬНО должна содержать предупреждения о критически важных для ЕГЭ моментах оформления:

1. ОДЗ (Область допустимых значений):
   - При наличии логарифмов: "Обязательно запишите ОДЗ для логарифма log(выражение) > 0"
   - При наличии корней четной степени: "Укажите ОДЗ для корня: подкоренное выражение ≥ 0"
   - При наличии дробей с переменной в знаменателе: "Выпишите ОДЗ: знаменатель ≠ 0"
   - При наличии тангенса или котангенса: "Запишите ОДЗ с учетом области определения"
   Формулировка: "В этой задаче присутствует [конкретный элемент], поэтому обязательно выпишите и учтите ОДЗ: [конкретное условие]"

2. МНОГОПУНКТОВЫЕ ЗАДАЧИ:
   - "Задача состоит из пунктов а) и б) - оформите решение и ответ для каждого пункта отдельно"
   - "Результат пункта а) можно использовать при решении пункта б)"

3. ФОРМАТ ОТВЕТА:
   - Для тригонометрических уравнений: "Ответ записывайте в виде x = ... + πn, n ∈ Z или x = ... + 2πn, n ∈ Z"
   - Для задач с параметром: "В ответе укажите все значения параметра с обоснованием"
   - При наличии единиц измерения: "В ответе обязательно укажите единицы измерения (метры, секунды, рубли и т.д.)"
   - Для точных значений: "Ответ давайте в точном виде: обыкновенная дробь, корень или выражение с π"

4. ОТБОР КОРНЕЙ (для пункта б):
   - "При отборе корней на отрезке покажите полное решение: подстановка в неравенство или изображение на тригонометрической окружности"
   - "Для отбора корней обязательно обоснуйте попадание каждого корня в заданный промежуток"

5. ГЕОМЕТРИЧЕСКИЕ ЗАДАЧИ:
   - "При нахождении угла между прямыми укажите острый угол (от 0° до 90°)"
   - "Обоснуйте каждый используемый факт (параллельность, перпендикулярность, равенство)"

6. НЕРАВЕНСТВА:
   - "Внимательно следите за строгостью неравенства (< или ≤) на всех этапах решения"
   - "При методе интервалов обязательно проверьте включение/исключение граничных точек"

7. ПОЛНОТА РЕШЕНИЯ:
   - "Записывайте все промежуточные вычисления и преобразования"
   - "Обосновывайте каждый логический переход словами или ссылками на теоремы"
   - "Для каждого шага, где используется формула или утверждение, обязательно указывайте её название (например: 'По теореме Пифагора...', 'Используя формулу площади треугольника...')"

ВАЖНО: Все указания должны быть конкретными и привязанными к условию задачи. Например: "Так как в задаче есть log₂(x-3), обязательно запишите ОДЗ: x-3 > 0, то есть x > 3"
"""
...
1. 1-2 предложения:
   ...
   - Напомните о формате ответа для ЕГЭ, а также об ОДЗ, если в задаче есть ограничения на переменную
...
1. 3-5 предложений:
   ...
   - Напомните о формате ответа: "Помните, что в ЕГЭ ответ записывается...", а также об ОДЗ, если в задаче есть ограничения на переменную
...
1. 5-8 предложений:
   ...
   - Напомните о ОДЗ, если в задаче есть ограничения на переменную
